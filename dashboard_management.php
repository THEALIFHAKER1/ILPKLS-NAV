...
if (isset($_POST["update"])) {
    ...
    echo $DATEE;
    echo $DATES;
        $result = mysqli_query($con,"UPDATE places SET STATUS='$STATUS',DATES='$DATES',DATEE='$DATEE' WHERE MANAGERID='$ID'");
    if ($result) { ?>
<main x-data="app">
...
<div class="flex flex-row max-[800px]:flex-col mt-5">
    <div class="flex-[50%] h-screen">
        <h1 class="text-xl text-white font-bold ml-10"><?php echo $info['NAMES']; ?></h1>
        <div class="relative center text-white"> 
            <img class="object-cover w-full h-[500px] rounded-[52px] p-7" src="data:image/jpg;charset=utf8;base64,<?php echo base64_encode($info["IMG"]); ?>" />
        </div>
    </div>
    <div class="h-screen w-full flex-[50%]">
        <h1 class="text-xl text-white font-bold ml-10">UPDATE MAKLUMAT</h1>
...
<div date-rangepicker class="flex items-center">
  ...
  <div class="relative">
    ...
    <input name="DATEE" required type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Select date end">
        ...
        <?php
        $IDPLACE = $info['ID'];
  $data = mysqli_query($con, "SELECT * FROM placesmemos WHERE PLACESID='$IDPLACE'");
  $rowcount=mysqli_num_rows($data);
  ?><h1 class="text-xl text-white font-bold ml-10 my-7">MEMOS [<?php echo $rowcount; ?>]</h1>
  <div class="h-auto w-full overflow-y-auto scrollbars">
    <?php $data = mysqli_query($con,"SELECT * FROM placesmemos WHERE PLACESID='$IDPLACE'");
