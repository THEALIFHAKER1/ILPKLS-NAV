...
<div class="flex flex-row h-screen max-[800px]:flex-col mt-5">
  ...
  <div class="overflow-y-auto h-screen scrollbars"><?php while (
    ...
    $info["IMG"]
			...
			<div class=" absolute top-3 text-black left-4 font-black px-2 bg-white rounded-lg"><?php echo $info[
       "NAMES"
   ]; ?></div>
			<div class=" absolute top-10 rounded-lg px-2 <?php if (
       $info["STATUS"] == "OPEN"
   ) {
       echo "bg-green-700";
   } else {
       echo "bg-red-700";
   } ?> left-4"><?php echo $info["STATUS"]; ?> <?php if (
     $info["STATUS"] == "CLOSE"
 ) {
     echo $info["DATES"];
 } ?>  <?php if ($info["STATUS"] == "CLOSE") {
      echo "> ";
      echo $info["DATEE"];
  } ?></div>
		</div>
  ...
  <div class="h-screen w-full overflow-y-auto scrollbars"><?php while (
      $info = mysqli_fetch_array($data)
  ) { ?>
    <?php
    $TAG = $info["TAG"];
